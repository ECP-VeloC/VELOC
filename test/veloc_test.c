...
//    sprintf(original, "veloc_test-file-ckpt-%d_%d.dat", v, rank);
    sprintf(original, "blah");
  ...
  if(VELOC_Checkpoint_wait() != VELOC_SUCCESS){
    printf("VELOC_Checkpoint_wait FAILED\n");
    return 1;
  }
...
printf("vvveloc_file is %s\n",veloc_file);
  ...
  timestep++;
//  int fd = open(veloc_file, O_WRONLY);
...
int fd;
FILE * file;
file = fopen(veloc_file, "r");
if (file){
   fclose(file);
   fd = open(veloc_file, O_TRUNC | O_WRONLY);
}else{
   fd = open(veloc_file, O_CREAT | O_TRUNC | O_WRONLY);
}
//  int fd = open(veloc_file, O_CREAT | O_TRUNC | O_WRONLY);
//exit(0);
char com[50];
sprintf(com, "chmod 777 %s", veloc_file);
system(com);
...
//  int fd = open(veloc_file, O_WRONLY);
  if(fd < 0){
printf("FD < 0 \n");
    fd = open(veloc_file, O_WRONLY);
  }
//  int fd = open("/g/g19/kosinov/scratch/veloc_test-0-0.dat", O_CREAT | O_TRUNC | O_WRONLY);
